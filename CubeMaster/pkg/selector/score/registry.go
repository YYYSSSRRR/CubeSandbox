...
// Factory builds a scoring plugin. Factories are registered at assembly time
...
// Register registers a scoring plugin under a unique name. Registering the same
// name twice is an error so a plugin can never silently replace a built-in one.
func Register(name string, factory Factory) error {
	if name == "" {
		return fmt.Errorf("score: register with empty name")
	}
	if factory == nil {
		return fmt.Errorf("score: register %q with nil factory", name)
	}
	regMu.Lock()
	defer regMu.Unlock()
	if _, dup := registry[name]; dup {
		return fmt.Errorf("score: plugin %q already registered", name)
	}
